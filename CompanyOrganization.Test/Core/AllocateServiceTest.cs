...
namespace CompanyEmployeesBalancing.Test.Core
{
    ...
    {
        private readonly AllocateService _allocateService;
        private readonly TeamService _teamService;
        
        public AllocateServiceTest()
        {
            _allocateService = DependencyResolver.Get<AllocateService>();
        ...
        {
            var exception = Assert.ThrowsException<Exception>(() => _allocateService.Allocate(new List<Team>(), new List<Employee>()));
            Assert.AreEqual(Messages.CompanyWithoutTeams, exception.Message);
        }
